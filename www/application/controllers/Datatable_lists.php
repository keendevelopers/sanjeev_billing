...
class Datatable_lists extends CI_Controller
{
    ...
    public function invoice_list()
    {
        ...
        foreach ($list as $invoice) {
           ...
           if($invoice->is_complete == '0'){
			 $buttons .= '<a class="btn btn-success btn-mini mr-l-5 kd-margin-bottom" onclick=view_modal("' . base_url() . 'product/modal/view_invoice_balance_details/' . $invoice->inv_id . '")><i class="fa fa-money"></i> </a>';
			}

			 $buttons .= '<a class="btn btn-info btn-mini mr-l-5 kd-margin-bottom" onclick=view_modal("' . base_url() . 'product/modal/view_invoice_balance_details_log/' . $invoice->inv_id . '")><i class="fa fa-align-left"></i> </a>';
